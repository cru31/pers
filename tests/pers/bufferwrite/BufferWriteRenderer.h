...
struct BufferWriteRendererConfig {
...
public:
    BufferWriteRenderer() = default;
    ~BufferWriteRenderer();
    
    // Initialize the renderer with instance and config
    bool initialize(const std::shared_ptr<pers::IInstance>& instance, const BufferWriteRendererConfig& config);
    ...
    // Create bufferwrite resources (vertex buffer, shaders, pipeline)
...
private:
    // Configuration
    BufferWriteRendererConfig _config;
    ...
    std::shared_ptr<pers::IBuffer> _indexBuffer;  // For indexed drawing
    uint32_t _indexCount = 0;  // Number of indices to draw
